...
// register I/O hooks
function str(x) {
  // convert a k string or symbol to a js string
  var s = conv.tojs(x);
  if (typeof s !== "string") {
  ...
  if (f) {
    ...
    if (dt == 0) {
      ...
      if (!fs.statSync(f).isDirectory()) {
        tojs = fs.readFileSync(f, "utf8");
      } else {
        throw Error("ERROR: Path '" + f + "' is a directory");
      }
    }
  } else if (rl) {
    ...
    if (dt == 0) {
      ...
      tojs = b.toString("utf8", 0, n);
    }
  }
  if (tojs) {
    try {
      return conv.tok(JSON.parse(tojs));
    } catch (err) {
      throw Error("JSON parsing error: " + err.message);
    }
  }
}
...
for (var i = 0; i < 2; i++) {
  ok.setIO("0:", i, function (x) {
    return readp(0, x);
  });
}
for (var i = 0; i < 2; i++) {
  ok.setIO("1:", i, function (x) {
    return readp(1, x);
  });
}
for (var i = 0; i < 2; i++) {
  ok.setIO("5:", 1, function (x) {
    return conv.tok(ok.format(x));
  });
}
for (var i = 2; i < 6; i++) {
  ok.setIO("0:", i, function (x, y) {
    return writep(0, x, y);
  });
}
for (var i = 2; i < 6; i++) {
  ok.setIO("1:", i, function (x, y) {
    return writep(1, x, y);
  });
}
...
process.stdout.write(
  "oK v" +
    ok.version +
    " (inspired by K5: http://kparc.com/k.txt; \\h for help)\n"
);
var rl = readline.createInterface({
  ...
  completer: function (line) {
    ...
    var prefix = m ? m[0] : "";
    var names = [];
    for (var e = env; e; e = e.p) {
      // iterate over ancestor environments
      for (var name in e.d) {
        if (
